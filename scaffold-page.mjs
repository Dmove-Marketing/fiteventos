...
function replaceImages(html) {
  ...
  const result = html.replace(/<img\s([^>]*?)(?:\s\/)?>/gi, (match, attrs) => {
    ...
    // Skip data URIs, SVGs, and tracking pixels (1×1)
    if (!src || src.startsWith('data:') || src.endsWith('.svg')) return match;
    if ((widthStr && parseInt(widthStr) <= 1) || (heightStr && parseInt(heightStr) <= 1)) return match;
    ...
    let props = `src="${basename}" alt="${alt}"`;
...
function replaceVideos(html) {
  ...
  const result = html.replace(/<video[\s\S]*?<\/video>/gi, (match) => {
    hasVideo = true;
    const srcMatch =
      match.match(/<source[^>]*\bsrc=["']([^"']*)["']/i) ||
      match.match(/\bsrc=["']([^"']*)["']/i);
    const src = srcMatch ? srcMatch[1] : '';
    const poster = (match.match(/\bposter=["']([^"']*)["']/) || [])[1] || '';
    const title =
      (match.match(/\btitle=["']([^"']*)["']/) || [])[1] ||
      (match.match(/\baria-label=["']([^"']*)["']/) || [])[1] ||
      'Vídeo';
    const cls = (match.match(/\bclass=["']([^"']*)["']/) || [])[1];

    let props = `src="${src}" poster="${poster}" title="${title}" description="" uploadDate="2025-01-01" duration="PT1M"`;
    if (cls) props += ` class="${cls}"`;
